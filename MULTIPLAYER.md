...
## 4-1. 좀비 공유 (클라이언트 권위 방식)

좀비는 **청크 소유권**으로 분산 시뮬레이션한다 (서버 부하 최소화):

- 각 클라이언트는 자기 주변 활성 청크의 소유권을 서버에서 받는다 (선착순).
- **소유한 청크의 좀비만** 로컬에서 스폰·AI·이동을 계산하고:
  - **스폰/사망 → 즉시 이벤트**, **위치 → 6Hz + 클라 보간**
- 다른 클라는 그 좀비를 `remote_zombies` 로 받아 보간 렌더링한다.
- **전투**: 원격(타 소유) 좀비를 쏘면 소유자에게 피해가 전달돼 처리(권위 일관성).
  원격 좀비가 내게 닿으면 접촉 피해를 입는다.
- 청크를 벗어나면 소유권을 반납 → 가까워진 다른 플레이어가 이어받는다.

> 서버는 **중계만** 하므로 ngrok + 일반 PC 로도 충분하다.
> NPC·시민·차량·아이템은 공유하지 않고 각 클라가 로컬 생성한다.
> ⚠ 모두 같은 지점에서 시작하므로 **시작 지역 좀비는 잠시 중복**될 수 있다(소유권 정리되며 안정화).

## 4-2. 최대 인원 제한

서버는 **동시 10명**까지만 받는다 (`server.py` 의 `MAX_PLAYERS`).
초과 접속은 거부되고, 거부된 클라이언트는 "서버 정원 초과" 알림 후 **싱글플레이로 진행**된다.
